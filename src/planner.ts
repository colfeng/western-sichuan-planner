...
export type PlanDay = { day: number; date: string; sunrise: string; sunset: string; daylightHours: number; departureTime: string; estimatedArrivalTime: string; daylightMarginMinutes: number; startAnchorId: string; endAnchorId: string; viaAnchorIds: string[]; driveHours: number; distanceKm: number; activityHours: number; restHours: number; mealHours: number; dutyHours: number; sleepAltitude: number; attractionIds: string[]; roads: string[]; legIds: string[]; evPlan?: EvDayPlan };
...
function plannedBreaks(driveHours: number, activityHours: number) { const restHours = driveHours >= 1.5 ? Math.ceil(driveHours / 2) * 0.25 : 0; const mealHours = driveHours + activityHours >= 5 ? 0.75 : 0; return { restHours: round(restHours), mealHours }; }
...
function makeOption(sourceInput: PlannerInput, strategy: Strategy): PlanOption {
  const input = normalizeInput(sourceInput); const required = unique(input.selectedAttractionIds).filter((id) => attractionById.has(id)); const requiredRoute = buildRoute(required, input); const routeAnchorSet = new Set([input.startAnchorId, input.endAnchorId, ...(requiredRoute?.flatMap((leg) => [leg.from, leg.to]) ?? [])]); const month = new Date(`${input.startDate}T12:00:00Z`).getUTCMonth() + 1; const theme = strategy === "culture" ? "culture" : strategy === "scenery" ? "scenery" : "rest"; const candidates = unique([
    ...strategySuggestions[strategy],
    ...attractions.filter((item) => routeAnchorSet.has(item.anchorId) && item.visitHours <= 3 && item.detourHours <= 0.8 && item.bestMonths.includes(month) && (item.themes.includes(theme) || strategy === "comfort")).map((item) => item.id),
  ]).filter((id) => !required.includes(id)); let suggested: string[] = []; let raw = rawPlan(input, strategy, required); const suggestionLimit = Math.min(8, Math.max(3, input.days));
  if (raw && !raw.warnings.some((item) => item.severity === "block")) for (const candidate of candidates) {
    if (suggested.length >= suggestionLimit) break; const trial = rawPlan(input, strategy, [...required, ...suggested, candidate]); if (!trial || trial.warnings.some((item) => item.severity === "block") || trial.schedule.some((day) => day.attractionIds.length > 3 || day.activityHours > 8.5)) continue; suggested.push(candidate); raw = trial;
  }
